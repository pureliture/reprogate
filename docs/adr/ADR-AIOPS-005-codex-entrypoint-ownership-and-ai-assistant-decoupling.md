# ADR-AIOPS-005: Codex Entrypoint Ownership 및 JetBrains AI Assistant Decoupling
...
`WP-AIOPS-2026-03-008`에서 Codex+OMX parity를 맞추기 위해 `$ai-ops` named entrypoint와
...
3. `$ai-ops` skill이 active WP(`WP-AIOPS-2026-03-008`)를 직접 참조하면서 thin adapter 원칙과 토큰 효율을 해쳤다.
...
- 이 결정은 hidden local adapter를 canonical 정책 저장소로 두지 않는다는 `ADR-AIOPS-004`와 일관된다.
...
- `WP-AIOPS-2026-03-008` P3/S1에서 실제 동작과 문서 정합성을 다시 검증한다.

## Related

- [WP-AIOPS-2026-03-008](../work-packets/WP-AIOPS-2026-03-008-codex-omx-alignment.md)
- [ADR-AIOPS-003](./ADR-AIOPS-003-conditional-team-activation-and-optout.md)
- [ADR-AIOPS-004](./ADR-AIOPS-004-ai-tool-artifact-boundary.md)
