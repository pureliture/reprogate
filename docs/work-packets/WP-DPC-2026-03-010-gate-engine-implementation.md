...
track: "reprogate core implementation"
---

# WP-DPC-2026-03-010: ReproGate Gate 엔진 구현

## 1. Background

ADR-007에서 OPA/Rego를 rules 엔진으로 채택했다. 이제 실제 Gate 엔진을 구현해야 한다.

Gate 엔진은 ReproGate의 **"보장(Hard)"** 레이어를 담당하며, 작업 기록과 산출물을 증거로 검사해 필요한 규칙을 물리적으로 차단한다.

## 2. Goal

OPA/Rego 기반 Gate 엔진을 구현하여 `dpc check` 및 adapter hook과 연동한다.
...
| **입력 스키마** | 어떤 필드가 제공되나? | trigger, file, context 외 records/artifacts 추가? |
...
records:
  # TODO: planning/decision/verification 등 어떤 증거를 제공할지
context:
  # TODO: 어떤 필드?
```
...
- [presets-spec.md](../design/presets-spec.md) - ReproGate preset / Skill 설계
- [architecture.md](../design/architecture.md) - ReproGate 아키텍처
- [final-definition.md](../strategy/final-definition.md) - ReproGate 기준 정의
