...
AI Ops separates reusable framework assets from project adapters and local runtime data.

## Artifact Classes

| Class | Purpose | Typical Location | Commit? |
| --- | --- | --- | --- |
| Framework | Reusable process docs, scripts, templates | `docs/`, `scripts/`, `config/`, `templates/` | Yes |
| Project Adapter | Project-specific entrypoints and record scaffolds | `AGENTS.md`, `.codex/`, `.claude/`, project changelog/index files | Yes |
...
3. Keep generated adapters traceable to `ai-ops.config.yaml`.
...
If a target repository should be able to run AI Ops without reading the source framework checkout, the generated output must include:
