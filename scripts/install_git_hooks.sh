...
cat > .claude/hooks/pretooluse-dpc-guard.py <<'EOF'
#!/usr/bin/env python3
import pathlib
import runpy
...
chmod +x .claude/hooks/pretooluse-dpc-guard.py
...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  echo "Installed dpc git hooks, configured core.hooksPath, and synced the Claude hook wrapper."
else
  echo "Installed dpc hook files and Claude hook wrapper. Skipped git config because this is not a git worktree."
fi
